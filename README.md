...
これはMVPとして公開しているツールです。まずは個人利用や、仕組みを理解している人が試す用途を想定しています。

まだChrome Web Store配布、macOSアプリ化、署名・notarization、正式なinstaller化はしていません。非エンジニア向けの本番配布を意識した設計・UXにはなっていません。
...
動作確認用のサンプル画像を `extension/samples/` に置いています。popupの `サンプルを表示` からも開けます。日本語と英語が混ざったUI、レシート、ラベル、ホワイトボード風の画像で、右クリックOCRと範囲OCRを試せます。
