...
APIキーは各ユーザーのMacのKeychainに保存し、Chrome拡張には保存しません。OpenAI APIクレジットを活用できる場合は、多言語・低解像度・モーダル表示・UIスクリーンショットなどのサンプルを増やし、モデル/プロンプト比較、token usage確認、リリース前のOCR回帰チェックに使うことで、実利用で壊れにくい公開MVPとして育てていきます。

## 現在の位置づけ

VisionClipは公開MVPです。Chrome拡張を手動で読み込み、Mac側のNative Messagingホストをインストールできる人が、個人利用や小さなチーム内検証で使うことを想定しています。

現時点の配布はGitHubからの手動セットアップです。Chrome Web Storeへのupload workflowとmacOS Native Messagingホストのpkg生成は用意していますが、Store公開はまだ行っていません。macOSアプリ化、Developer ID署名・notarization済み配布は未対応です。一般配布へ向けて残っている作業は [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) にまとめています。
...
### Native Messagingホストの状態をまとめて確認したい
...
- 現時点の配布はGitHubからの手動セットアップで、Developer ID署名・notarization済みinstallerではありません。
...
- macOSのDeveloper ID署名、notarization済み配布は未対応です。
...
Chrome Web Store用スクリーンショットは `assets/store/screenshots/` に置いています。生成元を更新した場合は、次で1280x800の提出候補を再生成できます。

```sh
./scripts/generate_store_screenshots.sh
```

Chrome Web Storeへのzip uploadは `.github/workflows/chrome-web-store.yml` から手動実行できます。初期方針はunlisted公開です。必要なGitHub Secretsや提出文面は [docs/STORE_LISTING.md](docs/STORE_LISTING.md) にまとめています。

macOS Native Messagingホストのpkgは、release artifact作成時に `dist/visionclip-native-host-macos-<arch>-v<version>.pkg` として生成されます。Storeの拡張IDを変える場合は `VISIONCLIP_RELEASE_EXTENSION_IDS` にスペース区切りで拡張IDを指定してください。pkg署名やnotarizationの環境変数は [docs/MACOS_DISTRIBUTION.md](docs/MACOS_DISTRIBUTION.md) にまとめています。
...
リリース前の実行順は [docs/RELEASE_RUNBOOK.md](docs/RELEASE_RUNBOOK.md)、確認項目は [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) にまとめています。
