...
if [[ $# -gt 0 ]]; then
  ...
  read -r -a EXPECTED_EXTENSION_IDS <<< "${VISIONCLIP_RELEASE_EXTENSION_IDS:-$DEFAULT_EXTENSION_ID}"
fi

for extension_id in "${EXPECTED_EXTENSION_IDS[@]}"; do
...
mkdir -p "$EXPANDED_DIR/payload"
(
  cd "$EXPANDED_DIR/payload"
  gzip -dc "$EXPANDED_DIR/expanded/Payload" | cpio -idm --quiet
)
...
node - "$EXPANDED_DIR/payload" "${EXPECTED_EXTENSION_IDS[@]}" <<'NODE'
const fs = require("fs");
const path = require("path");

const payloadDir = process.argv[2];
const extensionIds = process.argv.slice(3);
const manifestDir = path.join(payloadDir, "Library", "Google", "Chrome", "NativeMessagingHosts");

for (const hostName of ["com.mocchicc.visionclip", "com.mocchicc.image_ocr"]) {
  const manifestPath = path.join(manifestDir, `${hostName}.json`);
  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));

  if (manifest.name !== hostName) {
    throw new Error(`${manifestPath} has unexpected name: ${manifest.name}`);
  }
  if (manifest.type !== "stdio") {
    throw new Error(`${manifestPath} has unexpected type: ${manifest.type}`);
  }
  if (manifest.path !== "/Library/Application Support/VisionClip/visionclip-native-host") {
    throw new Error(`${manifestPath} has unexpected path: ${manifest.path}`);
  }
  for (const extensionId of extensionIds) {
    const expectedOrigin = `chrome-extension://${extensionId}/`;
    if (!Array.isArray(manifest.allowed_origins) || !manifest.allowed_origins.includes(expectedOrigin)) {
      throw new Error(`${manifestPath} is missing allowed origin: ${expectedOrigin}`);
    }
  }
}
NODE
