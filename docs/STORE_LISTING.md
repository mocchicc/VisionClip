...
Chrome Web StoreのPrivacyタブには、公開repo上のPrivacy Noticeを指定します。

```text
https://github.com/mocchicc/VisionClip/blob/main/docs/PRIVACY.md
```

default branch、repo名、公開先URLを変えた場合は、このURLも更新してください。
...
Chrome Web StoreのPrivacyタブでは、次の実態に合わせて申告します。
...
- Website content: ユーザーがOCR対象として選んだ画像、画像URL、または選択範囲スクリーンショットを扱います。OCR機能のためにOpenAI APIへHTTPSで送信します。
- Authentication information: ユーザー自身のOpenAI APIキーを扱います。APIキーはmacOS Keychainに保存し、OpenAI APIの認証にだけ使います。Chrome拡張local storageや開発者のサーバーには保存しません。
- User activity / browsing history: 継続的な閲覧履歴収集は行いません。VisionClipは、ユーザーがOCRを開始した現在のタブだけを対象にします。
- Extension storage: 選択中モデル、OCR履歴保存ON/OFF、直近の処理状態、保存ONの場合の最近5件のOCR結果プレビューとtoken usageだけをChrome local storageに保存します。

### Limited Use

VisionClipは、ユーザーデータをOCRとクリップボードコピー機能の提供に必要な範囲でだけ使います。広告、行動ターゲティング、信用判断、販売、またはVisionClipの単一目的と関係しない用途には使いません。開発者は独自サーバーでOCR対象画像、OCR結果、APIキー、閲覧履歴を収集・保存しません。
...
workflowは `./scripts/check.sh`、`./scripts/package_release.sh`、`node scripts/check_release_package.js`、Store資格情報preflightを通したextension zipだけをChrome Web Store API v2へ送ります。`publish=true` を選ばない限り、review submitは行いません。

`CWS_EXTENSION_ID` を既定値から変える場合は、macOS native host pkgの `allowed_origins` にも同じIDを含める必要があります。`VISIONCLIP_RELEASE_EXTENSION_IDS` にStore IDを入れて `./scripts/package_release.sh` を実行し、`./scripts/check_native_host_pkg.sh "$CWS_EXTENSION_ID"` で確認してください。
