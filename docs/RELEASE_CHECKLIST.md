...
- [x] 公開前にMIT Licenseの `LICENSE` を追加する
- [x] [LICENSE_DECISION.md](./LICENSE_DECISION.md) を確認し、選んだライセンス方針とREADME/告知文が一致している
...
- [ ] [RELEASE_RUNBOOK.md](./RELEASE_RUNBOOK.md) の手順が現在のworkflow、script、artifact名と一致している
...
- [ ] Chrome Web StoreのPrivacyタブに、Privacy Policy URL、Single purpose、User data usage、Limited Useを入力できる
- [ ] GitHub ActionsのDependabot設定とCodeQL workflowが有効になっている
- [ ] PR templateでprivacy、security、permissions、release impact、verificationを確認できる
...
- [ ] `node scripts/generate_release_qa_report.js` が成功し、`dist/release-qa-v<version>.md` に検証証跡を残せる
...
- [ ] `v<version>` タグでGitHub Releaseにartifactとrelease QA reportが添付され、`Release Artifacts` workflowの手動実行ではActions artifactが生成される
...
- [ ] `CWS_EXTENSION_ID` とnative host pkgの `allowed_origins` が一致している
- [x] `LICENSE` を追加した場合、native hostのrelease zipにも同梱されている
...
- [ ] `image-ocr-host diagnose <Chrome拡張ID>` がユーザー配下とsystem-wideのNative Messaging manifest状態を両方表示する
- [ ] OCR履歴の再コピーができる
- [ ] `履歴削除` でOCR履歴を削除できる
