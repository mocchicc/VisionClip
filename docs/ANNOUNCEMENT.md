...
VisionClipを公開するときのX投稿文と添付画像候補です。大げさな説明に寄せすぎず、普通に「使える小さなOSS」として伝わるトーンにしています。
...
I released VisionClip: a small open-source Chrome extension for macOS that OCRs images or selected screen regions and copies the text to your clipboard. BYOK with an OpenAI API key in macOS Keychain. Public MVP, manual setup for now.  
https://github.com/mocchicc/VisionClip
...
Shipping a public MVP of VisionClip today. It lets you OCR web images and arbitrary screen regions from Chrome, then copies the text to macOS clipboard. No store release yet, no installer yet, but the open-source core is ready to try.  
https://github.com/mocchicc/VisionClip
